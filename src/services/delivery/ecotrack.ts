...
export const ecotrackProvider: DeliveryProvider = {
    ...
    createOrder: async (order: Order, credentials?: DeliveryAPICredentials) => {
        ...
        // Commune names now come directly from EcoTrack's official API data,
        ...
        const payload: any = {
            ...
            adresse: (isOffice && order.stopDesk?.address) ? order.stopDesk.address : (order.customer.address || order.customer.commune),
            commune: targetCommune,
            code_wilaya: Number(wilayaCodeNum),
        ...
        try {
            ...
            if (response.status === 422 && data.errors) {
                const errorMsg = Object.entries(data.errors)
                    ...
                    .join(' | ');
                throw new Error(`Validation Error: ${errorMsg}\nPayload Commune: ${payload.commune}\nWilaya Code: ${payload.code_wilaya}`);
            }
