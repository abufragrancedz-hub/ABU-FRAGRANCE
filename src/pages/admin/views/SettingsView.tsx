...
export const SettingsView: React.FC = () => {
    ...
    const [marketingConfigs, setMarketingConfigs] = useState({ facebook_pixel_id: '', tiktok_pixel_id: '' });
    const { } = useShop();
    ...
    const fetchConfigs = async () => {
        try {
            ...
            const dConfig = res.data?.delivery_config || {};
            setConfigs(dConfig);

            // Handle Marketing
            const mConfig = res.data?.marketing || {};
            setMarketingConfigs({
                facebook_pixel_id: mConfig.facebook_pixel_id || '',
                tiktok_pixel_id: mConfig.tiktok_pixel_id || ''
            });
    ...
    const handleSave = async (carrierId: string) => {
        ...
        try {
            ...
            // In Cloudflare, delivery_config is stored as a single setting JSON blob
            await apiFetch('/api/admin/settings/delivery_config', {
                method: 'PUT',
                body: JSON.stringify(updatedDeliveryConfig)
            });
    ...
    const handleSaveMarketing = async () => {
        setSaving(true);
        try {
            await apiFetch('/api/admin/settings/marketing', {
                method: 'PUT',
                body: JSON.stringify({
                    ...marketingConfigs,
                    updatedAt: new Date().toISOString()
    ...
    const handleMarketingChange = (field: 'facebook_pixel_id' | 'tiktok_pixel_id', value: string) => {
        setMarketingConfigs(prev => ({ ...prev, [field]: value }));
    };
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-700">
            ...
            <div className="grid gap-8">
                ...
                {/* Marketing Settings */}
                <div className="bg-white rounded-[2rem] shadow-sm border border-primary/5 p-8 transition-all hover:shadow-md mt-6">
                    <div className="flex items-center justify-between mb-8 border-b border-primary/5 pb-6">
                        <div className="flex flex-col">
                            <h3 className="text-xl font-black text-primary uppercase tracking-wide">Marketing Pixels</h3>
                            <p className="text-xs font-bold text-secondary uppercase tracking-widest mt-1">Analytics & Tracking</p>
                        </div>
                        <button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="block text-xs font-black uppercase tracking-widest text-gray-400">Facebook Pixel ID</label>
                            <div className="relative group">
                                <input
                                    type="text"
                                    className="w-full px-5 py-3.5 bg-gray-50 border border-transparent focus:border-primary/20 focus:bg-white rounded-xl transition-all outline-none font-mono text-sm text-primary"
                                    placeholder="e.g. 1012134567890"
                                    value={marketingConfigs.facebook_pixel_id}
                                    onChange={e => handleMarketingChange('facebook_pixel_id', e.target.value)}
                                />
                            </div>
                        </div>

                        <div className="space-y-2">
                            <label className="block text-xs font-black uppercase tracking-widest text-gray-400">TikTok Pixel ID</label>
                            <div className="relative group">
                                <input
                                    type="text"
                                    className="w-full px-5 py-3.5 bg-gray-50 border border-transparent focus:border-primary/20 focus:bg-white rounded-xl transition-all outline-none font-mono text-sm text-primary"
                                    placeholder="e.g. CQUABCD123EFGH4"
                                    value={marketingConfigs.tiktok_pixel_id}
                                    onChange={e => handleMarketingChange('tiktok_pixel_id', e.target.value)}
                                />
                            </div>
                        </div>
                    </div>
