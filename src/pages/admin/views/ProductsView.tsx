...
import { Plus, Edit, Trash2, Search, X, Upload, Percent, Truck, Building2, Package } from 'lucide-react';
import { Product, ProductSize, QuantityDiscount, DeliveryCompany } from '../../../types';
...
export const ProductsView: FC<ProductsViewProps> = ({ products, deleteProduct, saveProduct, syncShopToCloudflare, isSaving }) => {
    ...
    const initialProductState: Partial<Product> = {
        name: '', description: '', price: 0, category: '', image: '', images: [], sizes: [], isPromo: false, oldPrice: 0, freeDelivery: false, deliveryCompany: 'anderson', stopDeskEnabled: false
    };
    ...
    const handleManualSync = async () => {
        setIsSyncing(true);
        try {
            await syncShopToCloudflare();
            alert("✅ Products successfully synced to Cloudflare Shield!");
        } catch (error: any) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    <button
                        ...
                        className="flex-1 sm:flex-none btn-primary flex items-center justify-center shrink-0 w-full sm:w-auto"
                    >
                ...
                {currentProducts.map((product) => (
                    <div key={product.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden hover:shadow-md transition-shadow group">
                        <div className="aspect-square relative overflow-hidden bg-gray-100 dark:bg-slate-700">
                            ...
                            {product.deliveryCompany === 'dhd' && (
                                <div className="absolute bottom-2 left-2 bg-purple-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 shadow-sm">
                                    <Package className="w-3 h-3" />
                                    DHD
                                </div>
                            )}
                            {product.deliveryCompany === 'both' && (
                                <div className="absolute bottom-2 left-2 bg-blue-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 shadow-sm">
                                    <Truck className="w-3 h-3" />
                                    MULTI
                                </div>
                            )}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5 md:space-y-6 pb-4">
                            ...
                            <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 space-y-3">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {([
                                        { value: 'anderson', label: 'Anderson', sublabel: 'EcoTrack', color: 'blue' },
                                        { value: 'dhd',      label: 'DHD',      sublabel: 'DHD Delivery', color: 'purple' },
                                        { value: 'both',     label: 'Both',     sublabel: 'Customer chooses', color: 'indigo' },
                                    ] as { value: DeliveryCompany; label: string; sublabel: string; color: string }[]).map(opt => (
                                        <button
                                            key={opt.value}
                                            type="button"
                                            onClick={() => setCurrentProduct({ ...currentProduct, deliveryCompany: opt.value })}
                                            className={`p-3 rounded-xl border-2 text-center transition-all ${
                                                currentProduct.deliveryCompany === opt.value
                                                    ? opt.value === 'dhd'
                                                        ...
                                                        : 'border-blue-600 bg-white text-blue-800 ring-2 ring-blue-200'
                                                    : 'border-gray-200 bg-white text-gray-500 hover:border-blue-300'
                                            }`}
                                        >
                                            <div className="font-bold text-sm">{opt.label}</div>
                                            <div className="text-[10px] mt-0.5 opacity-70">{opt.sublabel}</div>
                                        </button>
                                    ))}
                                </div>
                                {currentProduct.deliveryCompany === 'both' && (
                                    <p className="text-xs text-indigo-700 font-medium bg-indigo-50 px-3 py-2 rounded-lg border border-indigo-100">
                                        💡 Customers will see a dropdown to choose between Anderson or DHD at checkout.
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="flex items-center gap-3 bg-orange-50 p-3 rounded-xl border border-orange-100">
                                <input
                                    ...
                                    checked={currentProduct.stopDeskEnabled || false}
                                    onChange={e => setCurrentProduct({ ...currentProduct, stopDeskEnabled: e.target.checked })}
                                />
                                <label htmlFor="stopDeskEnabled" className="text-sm font-bold text-orange-800 select-none cursor-pointer flex items-center gap-2">
                                    <Building2 className="w-4 h-4" />
                                    Enable Stop Desk (Office Pickup) for this product
                                    <span className="text-[10px] font-medium text-orange-600 bg-orange-100 px-2 py-0.5 rounded-full">
                                        {currentProduct.stopDeskEnabled ? 'ON' : 'OFF'}
                                    </span>
                                </label>
                            </div>
            ...
            {productToDelete && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-primary/20 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl border-2 border-red-100 animate-in zoom-in-95 duration-200">
                        <div className="flex flex-col items-center text-center space-y-4">
                            <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center text-red-600">
                                <Trash2 className="w-8 h-8" />
                            </div>
                            <div className="space-y-2">
                                <h3 className="text-xl font-black text-blue-900 uppercase tracking-tight">Delete Product?</h3>
                            ...
                            <div className="flex w-full gap-3 pt-2">
                                <button
                                    ...
                                    className="flex-1 px-4 py-3 rounded-xl bg-gray-100 text-gray-600 font-bold hover:bg-gray-200 transition-colors"
                                >
                                    Cancel
                                </button>
                                <button
