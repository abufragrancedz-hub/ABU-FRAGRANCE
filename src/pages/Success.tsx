...
export const Success: React.FC = () => {
    ...
    // Tracking fires always — even when Meta's tool visits with no order state
    React.useEffect(() => {
        try {
            const orderValue = Number(order?.total) || 0;
            const orderItems = Array.isArray(order?.items) ? order!.items : [];

            // Debug log (only when real order exists)
            if (order) {
                console.log("ORDER on success page:", JSON.stringify(order, null, 2));
            }

            // Meta (Facebook) Purchase Event — always fires
            if (typeof window !== 'undefined' && (window as any).fbq) {
                (window as any).fbq('track', 'Purchase', {
                    value: orderValue,
                    currency: 'DZD',
                    content_ids: orderItems.map((i: any) => String(i.id || i.productId || '')),
                    content_type: 'product'
                });
            }

            // TikTok CompletePayment Event — always fires
            if (typeof window !== 'undefined' && (window as any).ttq) {
                (window as any).ttq.track('CompletePayment', {
                    ...
                    contents: orderItems.map((i: any) => ({
                        content_id: String(i.id || i.productId || ''),
                        content_name: i.name || '',
